...
const AudioSys = {
  ...
  // ---- 游戏音效 ----
  shoot() { if (this.throttle('shoot', 90)) return; this.noise({ dur: 0.08, vol: 0.05, f0: 2400, f1: 900, q: 2 }); },
  // 连击越高命中音调越高（核心爽感机关）
  hit(combo) {
    ...
    this.noise({ dur: 0.04, vol: 0.09, f0: 3200 * p, f1: 1400, q: 1.5 });
  },
  ...
  levelup() { [523, 659, 784, 1046].forEach((f, i) => this.tone({ type: 'triangle', f0: f, dur: 0.14, vol: 0.12, delay: i * 0.07 })); },
  // 剑吟长音：剑品晋升
  tierUp() {
  ...
  boom() { this.noise({ dur: 0.7, vol: 0.3, f0: 700, f1: 60, q: 0.7 }); this.tone({ type: 'sine', f0: 120, f1: 30, dur: 0.7, vol: 0.3 }); },
