...
> 最后更新：2026-08-29（V0.1 已交付，V0.2 Survivor 化已定案待实施）
...
## 8. 下一步：V0.2 Survivor 化改造方案（已定案，照此实施）

**目标**：俯视平面割草，参考《诛天剑侠》/《土豆兄弟》。竖屏 540×960 不变。

1. **备份**：竖版玩法代码复制到 `legacy-lane/`，然后原地改造；
2. **操作**：任意位置按住拖动 = 虚拟摇杆（相对位移定方向，画半透明摇杆底+头），360° 走位；键盘 WASD/方向键；玩家贴图不旋转；摄像机跟随玩家；
3. **地图**：无限平面——`tile_ch1.png` 按摄像机偏移取模平铺 + `deco_*` 装饰件按坐标哈希随机撒点（走回来位置不变）；
4. **技能系统**（核心）：
   - 初始主动「御剑术」：飞剑索敌，1→6 把，冷却 0.5s、伤害 12；
   - 主动池（可升 5 级）：剑气斩（扇形穿透波）/ 天雷诀（随机落雷）/ 环绕飞剑（绕体旋转）/ 万剑归宗（周期剑雨）/ 剑域罡气（周身光环持续伤害）；
   - 被动池：剑心(伤害%)/迅捷(冷却+移速%)/剑影(投射+1)/剑域(范围%)/长生诀(周期回血)/吸灵(拾取半径%)/会心(暴击)；
   - 三选一从「未学技能 + 已学技能升级 + 被动」混合抽卡，技能栏底部 UI 显示已学（主动 6 格 + 被动 7 格）；
5. **怪物**：摄像机视野外一圈随机刷、直线追击+简单分离防重叠；血量/密度按分钟曲线爬升（每 30s 全体系数×1.25）；45s 一精英（掉宝箱爆经验金币）；2:30 刷 Boss、3:00 斩 Boss 获胜（时长是配置项，留 5/10 分钟局扩展）；同屏上限 60；
6. **HUD 重做**：顶部正计时+等级条+金币；底部主动/被动技能栏；Boss 血条沿用；
7. **保留不动**：对象池/震屏顿帧/FX/音效/存档/广告点位/结算翻倍；
8. **数值起点**：玩家 HP100 移速 220 拾取 90；狼妖 HP24（保持"三剑内死"的手感约定）；升级需求沿用 `4+3×Lv`；
9. **验收**：浏览器完整打一局 3 分钟（用泵帧脚本），指标：60fps、次留爽感（连斩不断、10 秒一次升级）、无报错。
